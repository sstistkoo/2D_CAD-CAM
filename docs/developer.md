...
| `cam/ops/roughLong.js` | Generátor průchodů — PODÉLNÉ hrubování (zleva = totéž zrcadleně přes `zMirror`). Pomocné vrstvy jsou v `ops/long/` a skládají se v PEVNÉM pořadí: `runScan` → `depthTabs` → `residualGuard` → `holderFit` → `entryRamp` → `intervalScan`. Kulatá destička — POSLEDNÍ VRSTVA NA DNĚ (pravidlo 3, `collectFloors`): dna = vodorovné úseky offsetové dráhy a nejnižší body vydutých oblouků mezi hloubkou a poslední vrstvou; dno v MISCE (z obou stran stoupá offset k poslední vrstvě) se vloží do žebříku PŘED každou hloubku a jede jen ve svém okně (`floorWin`), dno mimo misku (plošina) až když hloubka pod ním nevydá nic — jinak ho objede dojezd „bez schodků“. Nájezd po kontuře se zkrátí na ocas pod mělčí vrstvou (`trimLeadInTails`), když jeho začátek leží na už projeté dráze. Intervaly před kotvou `zCap` se u kulaté nezahazují (kus, který celý projede dobírací řetěz ramp, se zahodí až po něm) |
...
| `cam/ops/long/airPieces.js` | `splitPocketsAtAir()` — kapsa za hrbem přes vzduch → kusy se vjezdem ze vzduchu (pravidlo 7). U kulaté destičky dostává průsečíky polotovaru pro KRUŽNICI nosu (`stockCrossingsCircleAt` v `roughLong.js`: silueta rozšířená o R) a s `airLead` najede ze vzduchu i jediný kus materiálu, před kterým je začátek intervalu celý ve vzduchu — blok za stěnou, kde vrstva končí, bere bok nosu |
| `cam/ops/long/humpOrder.js` | `orderByHumps()` — pořadí podle pravidla 7 (nejdřív pravá strana hrbu). Přesunutému průchodu se nájezd po kontuře u kulaté destičky nezahodí celý: zůstane jeho ocas pod mělčí vrstvou (`leadInTail`, `tailStep`), zbytek by jel po obrobeném boku |
