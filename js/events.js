...
drawCanvas.addEventListener("mouseup", (e) => {
  ...
  // Kóta: dokončit přesné umístění kóty délky uvolněním myši (2. akce)
  if (state.tool === 'dimension' && state._dimFirstLine && state._dimPlacing) {
    finalizeDimPlacement(state.mouse.x, state.mouse.y);
  }
});
...
export function handleCanvasClick(wx, wy) {
  switch (state.tool) {
    ...
    case "fill": {
      const fillObj = handleFillAreaClick(wx, wy);
      if (fillObj && bridge.openFillColorPicker) bridge.openFillColorPicker(fillObj);
      break;
    }
    ...
    case "gearpair":
      handleGearPairClick(wx, wy);
      break;

    case "slot":
      handleSlotClick(wx, wy);
      break;

    case "polygon":
      handlePolygonClick(wx, wy);
      break;

    case "star":
      handleStarClick(wx, wy);
      break;
