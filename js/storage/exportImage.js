...
/** Vrátí jen objekty z viditelných vrstev. */
...
function exportSVG(background) {
  ...
  // Fallback na původní DOM generátor, pokud Maker.js není dostupný / prázdný model.
  if (!built) {
    exportSVGLegacy(background);
    return;
  }

  const { mk, model, layerOptions } = built;
...
function exportSVGLegacy(background) {
